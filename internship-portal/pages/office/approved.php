            ...
            <tbody>
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        // Modify the link for downloading the PDF
                        $downloadLink = (!empty($row["StudentName"])) ? "../tpo/letter.php?ID=" . $row["ID"] : "../tpo/group_letter.php?ID=" . $row["ID"];
                        echo "<td class='pt-3 text-center'>";
                        echo "<a href='" . $downloadLink . "' class='btn btn-warning' download>";
