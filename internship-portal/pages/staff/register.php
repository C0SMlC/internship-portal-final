...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty(trim($_POST["username"]))) {
    ...
    if (!ctype_alnum(str_replace(array("@", "-", "_"), "", $username))) {
      ...
      $sql = "SELECT fac_id FROM faculty_panel WHERE username = ?";
  ...
  if (empty($fullname_err) && empty($username_err) && empty($email_err) && empty($password_err) && empty($address_err) && empty($age_err) && empty($mobile_err)) {
    ...
    $sql = "INSERT INTO faculty_panel(fac_name, username, fac_email, password, fac_address, fac_age, fac_mobile) VALUES (?, ?, ?, ?, ?, ?, ?)";

    if ($stmt = mysqli_prepare($link, $sql)) {
      # Bind varibales to the prepared statement as parameters
      mysqli_stmt_bind_param($stmt, "sssssii", $param_fullname, $param_username, $param_email, $param_password, $param_address, $param_age, $param_mobile);
